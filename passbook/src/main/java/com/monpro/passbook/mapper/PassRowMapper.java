...
public class PassRowMapper implements RowMapper<Pass> {
    ...
    private static byte[] CONSUMER_DATE = Constants.PassTable.CON_DATE.getBytes();
    ...
    public Pass mapRow(Result result, int i) throws Exception {
        ...
        String conDateStr = Bytes.toString(result.getValue(FAMILY_I, CONSUMER_DATE));
        if(conDateStr.equals("-1")){
            pass.setConsumerDate(null);
        }else {
            pass.setConsumerDate(DateUtils.parseDate(conDateStr,patterns));
        }
        return pass;
    }
}
